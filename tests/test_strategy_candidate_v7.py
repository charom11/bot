...
from strategy_candidate_v7 import (
    LONG, SHORT, FLAT, EXIT,
    MAX_LEVERAGE, DEFAULT_RISK_PCT,
    resample_ohlcv_to_1h, classify_market_regime, calculate_v7_quality_score,
    _atr, structure_stop, adaptive_pullback_limit, choose_leverage,
    risk_based_notional, trailing_stop, manage_position,
    calculate_drawdown_multiplier, calculate_cost_budget,
    calculate_expected_net_r, generate_strategy_signal, backtest_frame,
...
def test_classify_market_regime():
    # 1. High Volatility & Breakdown
    assert classify_market_regime(btc_score=0.0, market_breadth=0.15, avg_market_atr_pct=0.050, btc_realized_vol=0.02) == "BREAKDOWN"
    assert classify_market_regime(btc_score=0.0, market_breadth=0.50, avg_market_atr_pct=0.050, btc_realized_vol=0.02) == "EXTREME_VOL"
    assert classify_market_regime(btc_score=0.0, market_breadth=0.50, avg_market_atr_pct=0.020, btc_realized_vol=0.04) == "EXTREME_VOL"

    # 2. Risk Off
    assert classify_market_regime(btc_score=-0.40, market_breadth=0.55, avg_market_atr_pct=0.020, btc_realized_vol=0.01) == "RISK_OFF"
    assert classify_market_regime(btc_score=0.10, market_breadth=0.35, avg_market_atr_pct=0.020, btc_realized_vol=0.01) == "RISK_OFF"

    # 3. Risk On
    assert classify_market_regime(btc_score=0.45, market_breadth=0.70, avg_market_atr_pct=0.020, btc_realized_vol=0.01) == "RISK_ON"

    # 4. Neutral
    assert classify_market_regime(btc_score=0.10, market_breadth=0.50, avg_market_atr_pct=0.020, btc_realized_vol=0.01) == "NEUTRAL"


def test_calculate_v7_quality_score():
    score_high = calculate_v7_quality_score(
        expected_net_r=1.5,
        stability=0.8,
        profit_factor=1.5,
        trend_quality=0.8,
    ...
    score_low = calculate_v7_quality_score(
        expected_net_r=-0.5,
        stability=0.2,
        profit_factor=0.6,
        trend_quality=-0.5,
...
def test_market_regime_filtering_in_signal():
    d = make_ohlcv_15m(n=300)
    # Market breakdown should halt all entries
    sig_bd = generate_strategy_signal(d, market_regime="BREAKDOWN")
    assert sig_bd["signal"] == FLAT
    assert any("market-wide breakdown" in r for r in sig_bd["reasons"])

    # Risk-off market should halt longs
    sig_ro = generate_strategy_signal(d, market_regime="RISK_OFF")
    if sig_ro["score"] > 0:
        assert sig_ro["signal"] == FLAT
        assert any("risk-off market restricts long" in r for r in sig_ro["reasons"])
