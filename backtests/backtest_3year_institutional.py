...
"""
==========================================================================================
⚡ 3-YEAR INSTITUTIONAL COMPREHENSIVE BACKTEST (2023 - 2026)
==========================================================================================
Full 36-Month (3-Year) Historical Backtest of the Production Live Strategy:
- Assets: 10 Core Perpetual Markets (BTC, ETH, SOL, LINK, AVAX, XRP, ADA, DOGE, NEAR, SUI)
- Timeframe: 15m Execution Bars (~105,120 Bars per asset across 3 Years)
- Starting Balance: User Live Tier ($10.49 USDT) & Standard $100 Reference Tier
- Sizing: Dynamic Margin 3.0% per trade | 50x Isolated Leverage
- Alpha Channels:
  1. 🏛️ Market Structure Shift (MSS / CHoCH Fractal Breakout)
  2. 🌪️ 5-MA Stack Momentum Consensus (9/20/50/100/200 Stack Alignment)
  3. 📐 Fibonacci Multi-Tier Harmonic OTE (0.618 - 0.786 - 0.886)
- Risk Gates: 3h Anti-Churn Cooldown, ADX(14) >= 22.0, BTC Dump Guard, Max 5 Positions, Max 3 Directional
...
# --------------------------------------------------------------------------
# 📥 3-Year Historical Data Downloader & Caching Engine
# --------------------------------------------------------------------------
def fetch_3year_klines_from_binance(symbol, start_str="2023-09-01", interval="15m"):
    cache_file = os.path.join(CACHE_DIR, f"{symbol}_{interval}_3year_{start_str}.csv")
    ...
    print(f"[DOWNLOADING 3Y DATA] {symbol} {interval} from {start_str} to present...", flush=True)
    all_rows = []
    curr_ts = start_ts
    limit = 1500
    ...
    print(f"[CACHED 3Y DATA] {symbol}: {len(df):,} bars cached ({df['open_time'].iloc[0].strftime('%Y-%m-%d')} to {df['open_time'].iloc[-1].strftime('%Y-%m-%d')})", flush=True)
    return df

# --------------------------------------------------------------------------
# 🧮 Fast Quantitative Indicator Calculations
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# ⚡ Precompute Signals for All 3 Channels
# --------------------------------------------------------------------------
def precompute_3year_signals(df, window=4):
    ...
    # 5-MA Bull/Bear Stack Alignment
    ...
    for i in range(50, n):
        ...
        # 1. Consensus Momentum Impulse (5-MA Stack + Vol Surge + ATR Expansion)
        ...
        # 2. Market Structure Shift (MSS Break of Swing Pivot with Vol Surge + EMA50/200)
        ...
        if impulse >= (1.5 * curr_atr):
            # Bullish pullback into 0.618 - 0.886
            ...
            # Bearish pullback into 0.618 - 0.886
...
# --------------------------------------------------------------------------
# 🚀 3-Year Unified Execution Simulation Engine
# --------------------------------------------------------------------------
def run_3year_simulation(
    ...
    max_positions=5,
    max_directional=3,
    cooldown_bars=12,  # 3.0 Hours (12 x 15m)
    min_adx=22.0,
    max_notional_cap=1000.0,
    fixed_sizing=False
):
    print("=" * 95)
    print(f" ⚡ INITIALIZING 3-YEAR UNIFIED INSTITUTIONAL BACKTEST (2023 - 2026)")
    print("=" * 95)
    print(f" • Starting Balance:    ${initial_balance:.2f} USDT")
    print(f" • Sizing Mode:         {'FIXED BASE ($15.75 Notional)' if fixed_sizing else f'3.0% Margin Per Trade | 50x Isolated (Capped @ ${max_notional_cap:,.0f} Max Order)'}")
    print(f" • Max Active Slots:    {max_positions} Concurrent Positions (Max {max_directional} Longs / Shorts)")
    print(f" • Anti-Churn Guard:    {cooldown_bars * 15 / 60:.1f} Hours ({cooldown_bars} Bars Cooldown)")
    print(f" • Alpha Channels:      MSS Shift + 5-MA Consensus + Fibonacci 0.618/0.786/0.886 OTE")
    ...
    for sym in SYMBOLS:
        ...
        if df is not None and len(df) > 500:
            data_map[sym] = df
            signals_map[sym] = precompute_3year_signals(df)
        ...
        if len(active_positions) < max_positions and balance > 1.0:
            for sym in SYMBOLS:
                ...
                if action is not None:
                    if fixed_sizing:
                        notional = 15.75  # Fixed based on $10.49 initial wallet tier
                        margin_alloc = notional / leverage
                    else:
                        margin_alloc = balance * margin_pct
                        notional = min(margin_alloc * leverage, max_notional_cap)
                        if notional < 5.00:
                            notional = 5.00
                        margin_alloc = notional / leverage
    ...
    annualized_roi = ((balance / initial_balance) ** (1.0 / 3.0) - 1.0) * 100.0

    print("\n" + "=" * 95)
    print(" 🏆 3-YEAR COMPREHENSIVE PERFORMANCE SCORECARD (2023 - 2026)")
    print("=" * 95)
    ...
    print(f" • Compound Annual ROI:  {annualized_roi:>+10.2f}% per year (3.0 Years)")
    ...
    print("-" * 95)
    for ch in ['MSS_SHIFT', '5MA_CONSENSUS', 'FIBONACCI']:
    ...
    print("\n" + "=" * 95)
    print(" 📅 36-MONTH MONTHLY PnL BREAKDOWN:")
    print("=" * 95)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="3-Year Institutional Backtest (2023 - 2026)")
    parser.add_argument("--balance", type=float, default=10.49, help="Starting balance (default: 10.49 USDT)")
    parser.add_argument("--fixed-sizing", action="store_true", help="Use fixed $15.75 notional sizing without exponential reinvestment")
    parser.add_argument("--max-notional", type=float, default=1000.0, help="Max position notional cap (default: $1000)")
    args = parser.parse_args()

    run_3year_simulation(initial_balance=args.balance, fixed_sizing=args.fixed_sizing, max_notional_cap=args.max_notional)
