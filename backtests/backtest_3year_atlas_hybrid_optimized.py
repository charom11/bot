...
"""
==========================================================================================
⚡ 3-YEAR ATLAS-HYBRID OPTIMIZED INSTITUTIONAL BACKTEST (2023 - 2026)
==========================================================================================
Carefully tuned hybrid combining Current Settings with ATLAS principles:
1. 🧬 Calibrated Darwinian Weights (Rewards top alpha channels, floor at 0.8x)
2. ⚖️ JANUS Macro Trend Shield (Enforces multi-timeframe BTC macro alignment)
3. 🛡️ Adversarial Structural R:R Filter (Guarantees >= 1.8x structural reward-to-risk)
4. 🔒 3-Stage Breakeven Scale-Out Engine (50% TP1 @ 1.5x ATR -> Breakeven -> 1.2x ATR Trailing)
==========================================================================================
"""
...
class CalibratedDarwinianWeights:
    def __init__(self, channels=['FIBONACCI', '5MA_CONSENSUS', 'MSS_SHIFT']):
        self.weights = {ch: 1.0 for ch in channels}
        self.channel_history = {ch: deque(maxlen=300) for ch in channels}
    ...
    def update_weights(self, current_bar):
        ...
        for ch, hist in self.channel_history.items():
            ...
            wr = wins / len(hist)
            tot_pnl = sum(hist)
            if wr >= 0.52 and tot_pnl > 0:
                self.weights[ch] = min(2.0, self.weights[ch] * 1.05)
            elif wr < 0.45 or tot_pnl < 0:
                self.weights[ch] = max(0.7, self.weights[ch] * 0.95)
...
def run_hybrid_simulation(initial_balance=100.0, max_positions=5, max_directional=3, margin_pct=0.03, leverage=50):
    print("=" * 95)
    print(" 🚀 RUNNING 3-YEAR CALIBRATED ATLAS-HYBRID INSTITUTIONAL BACKTEST (2023 - 2026)")
    print("=" * 95)
    ...
    darwin = CalibratedDarwinianWeights()
        ...
        if len(active_positions) < max_positions and balance > 1.0:
            for sym in SYMBOLS:
                ...
                if action is not None:
                    # CRO overextension check
                    if abs(c_price - sig['ema50']) > (3.0 * curr_atr):
                        continue

                    # Darwinian allocation
    ...
    print("\n" + "=" * 95)
    print(" 🏆 3-YEAR CALIBRATED ATLAS-HYBRID INSTITUTIONAL SCORECARD (2023 - 2026)")
    print("=" * 95)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="3-Year ATLAS Hybrid Optimized Backtest")
